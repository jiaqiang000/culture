...
export default function CourseDetail() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "syllabus" | "notes" | "related">("overview");
            ...
            <div className="p-6">
              ...
              {activeTab === "syllabus" && (
                <ul className="space-y-2">
                  {detail.chapters.map((c, idx) => (
                    <li key={c.id} className="flex items-center justify-between px-4 py-2 rounded-lg border border-slate-200">
                      <div className="text-slate-700">{idx + 1}. {c.title}</div>
                      <div className="text-xs text-slate-500">{c.type === "video" ? "视频" : "图文"}</div>
                    </li>
                  ))}
                </ul>
              ...
              {activeTab === "related" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {popularCourses
                    .filter((c) => detail.related?.includes(c.id))
                    .map((c) => (
                      <Link key={c.id} to={`/courses/${c.id}`} className="rounded-xl overflow-hidden border border-slate-200 hover:shadow-lg transition-all bg-white">
                        ...
                        <div className="p-3">
                          ...
                          <div className="text-xs text-slate-500 mt-1">{c.chapters} 课时 · {c.enrolled.toLocaleString()} 人</div>
                        </div>
                      </Link>
                    ))}
                </div>
